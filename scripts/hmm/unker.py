...
class AbstractUnker:
  ...
  """ Process the corpus and return a corpus with UNKs substituted """
...
class PratyayaUnker(AbstractUnker):
  ...
  def _abstractGuard(self):
    ...
    _am, _aM, Uam = "am", "a\xe1\xb9\x83", "*Uam*" # 1s/2s
    ...
    wordEnds = lambda w,s: word[-len(s):] == s

    self._ruleList = [(lambda w: wordEnds(w, _asya), Uasya),
...
class BasicUnker(AbstractUnker):
  ...
  def _abstractGuard(self):
    ...
    self._ruleList = [(lambda w: True, "*U*")]
